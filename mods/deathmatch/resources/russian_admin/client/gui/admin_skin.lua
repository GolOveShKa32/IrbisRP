...
function aPlayerSkin ( player )
	if ( aSkinForm == nil ) then
		...
		aSkinForm		= guiCreateWindow ( x / 2 - 140, y / 2 - 125, 280, 250, "Выбор скина для игрока", false )
		aSkinLabel		= guiCreateLabel ( 0.03, 0.09, 0.94, 0.07, "Выбор скина из списка или ввод id", true, aSkinForm )
					  guiLabelSetHorizontalAlign ( aSkinLabel, "center" )
					  guiLabelSetColor ( aSkinLabel, 255, 0, 0 )
		aSkinGroups		= guiCreateCheckBox ( 0.03, 0.90, 0.70, 0.09, "Сортировка по группам", false, true, aSkinForm )
		...
		aSkinAccept		= guiCreateButton ( 0.75, 0.28, 0.27, 0.09, "Выбрать", true, aSkinForm, "setskin" )
		aSkinCancel		= guiCreateButton ( 0.75, 0.88, 0.27, 0.09, "Отмена", true, aSkinForm )
		...
		if ( source == aSkinAccept ) then
			if ( tonumber ( guiGetText ( aSkinID ) ) ) then
				...
				if ( guiGridListGetSelectedItem ( aSkinList ) ~= -1 ) then
					...
					aMessageBox ( "Внимание", "Для игрока не выбран ID!" )
				end
			end
