...
function aSpectate ( player )
	if ( player == getLocalPlayer() ) then
		aMessageBox ( "Ошибка", "Невозможно наблюдать за собой" )
		return
	end
...
function aSpectator.Initialize ()
	if ( aSpectator.Actions == nil ) then
		...
		aSpectator.Freeze			= guiCreateButton ( 0.10, 0.21, 0.80, 0.05, "Заморозить", true, aSpectator.Actions )
		...
		aSpectator.Slap			= guiCreateButton ( 0.10, 0.51, 0.80, 0.05, "Ударить! "..aCurrentSlap.."hp", true, aSpectator.Actions )
		...
		aSpectator.PlayerList		= guiCreateGridList ( 0.03, 0.07, 0.94, 0.92, true, aSpectator.Players )
					  		  guiGridListAddColumn( aSpectator.PlayerList, "Имя игрока", 0.85 )
...
function aSpectator.SwitchPlayer ( inc, arg, inc2 )
	...
	if ( #players <= 0 ) then
		aMessageBox ( "Вопрос", "Не за кем наблюдать, выйти?", "aSpectator.Close ( false )" )
		return
	end
	local current = 1
...
function aSpectator.Render ()
	...
	if ( not aSpectator.Spectating ) then
		dxDrawText ( "Никто не следит", sx - 170, 200, sx - 170, 200, tocolor ( 255, 0, 0, 255 ), 1 )
		return
	end
	...
	if ( _DEBUG ) then
		...
		if ( isCursorShowing () ) then
			dxDrawText ( "Tip: mouse2 - свободный обзор", 20, sy - 50, 20, sy - 50, tocolor ( 255, 255, 255, 255 ), 1 )
		else
			dxDrawText ( "Tip: Колесы мыши для изменения масшатаба", 20, sy - 50, 20, sy - 50, tocolor ( 255, 255, 255, 255 ), 1 )
		end
	end
end
