...
function aViewMessage ( id )
	if ( aViewMessageForm == nil ) then
		...
		aViewMessageForm	= guiCreateWindow ( x / 2 - 150, y / 2 - 125, 300, 250, "", false )
					   guiCreateLabel ( 0.05, 0.10, 0.30, 0.09, "Категория:", true, aViewMessageForm )
					   guiCreateLabel ( 0.05, 0.18, 0.30, 0.09, "Тема:", true, aViewMessageForm )
					   guiCreateLabel ( 0.05, 0.26, 0.30, 0.09, "Время:", true, aViewMessageForm )
					   guiCreateLabel ( 0.05, 0.34, 0.30, 0.09, "От кого:", true, aViewMessageForm )
		...
		aViewMessageCloseB	= guiCreateButton ( 0.77, 0.88, 0.20, 0.09, "Закрыть", true, aViewMessageForm )
