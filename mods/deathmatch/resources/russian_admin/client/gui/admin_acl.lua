...
function aManageACL ()
	if ( aAclForm == nil ) then
		...
		aAclForm		= guiCreateWindow ( x / 2 - 230, y / 2 - 250, 460, 500, "ACL Менеджер", false )
		...
		aACLCreateGroup	= guiCreateButton ( 0.55, 0.05, 0.40, 0.04, "Создать группу", true, aAclForm )
		aACLCreateACL		= guiCreateButton ( 0.55, 0.10, 0.40, 0.04, "Создать ACL", true, aAclForm )
		...
		aACLDestroyGroup	= guiCreateButton ( 0.55, 0.25, 0.40, 0.04, "Удалить группу", true, aAclForm )
		aACLDestroyACL	= guiCreateButton ( 0.55, 0.25, 0.40, 0.04, "Удалить ACL", true, aAclForm )
		aACLAddObject		= guiCreateButton ( 0.55, 0.30, 0.40, 0.04, "Добавить пользователя", true, aAclForm )
		aACLRemoveObject	= guiCreateButton ( 0.55, 0.35, 0.40, 0.04, "Удалить пользователя", true, aAclForm )
		aACLAddACL		= guiCreateButton ( 0.55, 0.40, 0.40, 0.04, "Добавить ACL", true, aAclForm )
		aACLRemoveACL	= guiCreateButton ( 0.55, 0.45, 0.40, 0.04, "Удалить ACL", true, aAclForm )
		...
		aACLOk		= guiCreateButton ( 0.55, 0.40, 0.19, 0.04, "ОК", true, aAclForm )
		aACLCancel		= guiCreateButton ( 0.76, 0.40, 0.19, 0.04, "Отмена", true, aAclForm )

		aACLAddRight		= guiCreateButton ( 0.55, 0.30, 0.40, 0.04, "Добавить права", true, aAclForm )
		aACLExit		= guiCreateButton ( 0.75, 0.90, 0.27, 0.04, "Закрыть", true, aAclForm )
		aclDisplayOptions ( "", "" )
...
function aAdminACL ( type, acltable )
	...
	if ( type == "aclgroups" ) then
		...
		guiGridListSetItemText ( aACLList, aAclData["group_row"], 2, "Группы:", true, false )
	...
	elseif ( type == "aclobjects" ) then
		...
		for i, group in ipairs ( aAclData["groups"] ) do
			...
			if ( group == acltable["name"] ) then
				...
				guiGridListSetItemText ( aACLList, aAclData["objects_row"], 2, "  пользователи:", true, false )
	...
	elseif ( type == "aclrights" ) then
		...
		for i, acl in ipairs ( aAclData["acl"] ) do
			...
			if ( acl == acltable["name"] ) then
				...
				guiGridListSetItemText ( aACLList, aAclData["rights_row"], 2, "  права:", true, false )
...
function aClientACLClick ( button )
	...
	if ( button == "left" ) then
		...
		elseif ( source == aACLCreateGroup ) then
			aInputBox ( "Создание ACL группы", "Введите название группы:", "", "triggerServerEvent ( \"aAdmin\", getLocalPlayer(), \"aclcreate\", \"group\", $value )" )
		elseif ( source == aACLCreateACL ) then
			aInputBox ( "Создание ACL", "Введите ACL название:", "", "triggerServerEvent ( \"aAdmin\", getLocalPlayer(), \"aclcreate\", \"acl\", $value )" )
		elseif ( source == aACLAddObject ) then
			aInputBox ( "Создание ACL группы", "Введите имя пользователя:", "", "triggerServerEvent ( \"aAdmin\", getLocalPlayer(), \"acladd\", \"object\", \""..aAclData["current"].."\", $value )" )
		elseif ( source == aACLAddRight ) then
			aInputBox ( "Создание ACL", "Введите права:", "", "triggerServerEvent ( \"aAdmin\", getLocalPlayer(), \"acladd\", \"right\", \""..aAclData["current"].."\", $value )" )
		elseif ( source == aACLDestroyGroup ) then
			aMessageBox ( "Внимание", "Удалить "..aAclData["current"].." группу?", "triggerServerEvent ( \"aAdmin\", getLocalPlayer(), \"acldestroy\", \"group\", \""..aAclData["current"].."\" )" )
		elseif ( source == aACLDestroyACL ) then
			aMessageBox ( "Внимание", "Удалить "..aAclData["current"].." ACL?", "triggerServerEvent ( \"aAdmin\", getLocalPlayer(), \"acldestroy\", \"acl\", \""..aAclData["current"].."\" )" )
