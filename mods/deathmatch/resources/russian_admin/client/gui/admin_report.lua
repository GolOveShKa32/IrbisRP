...
function aReport ( player )
	if ( aReportForm == nil ) then
		...
		aReportForm		= guiCreateWindow ( x / 2 - 150, y / 2 - 150, 300, 300, "Связь с админом", false )
	...
	if ( button == "left" ) then
		if ( source == aReportAccept ) then
			if ( ( string.len ( guiGetText ( aReportSubject ) ) < 1 ) or ( string.len ( guiGetText ( aReportMessage ) ) < 5 ) ) then
				aMessageBox ( "Ошибка", "Тема/сообщение отсутствует." )
			else
				aMessageBox ( "Информация", "Сообщение отправлено." )
