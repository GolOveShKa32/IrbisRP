...
function createMapTab()
	aTabMap = {}
	aTabMap.Tab = guiCreateTab ( "Карты", aTabPanel, "maps" )
	...
	aTabMap.MapList = guiCreateGridList ( 0.03, 0.10, 0.35, 0.80, true, aTabMap.Tab )
					  guiGridListAddColumn( aTabMap.MapList, "Название карты", 2)
					  guiGridListAddColumn( aTabMap.MapList, "Название ресурса", 1)
					  guiGridListAddColumn( aTabMap.MapList, "Мод", 0.5)
	aTabMap.Start = guiCreateButton ( 0.45, 0.05, 0.3, 0.04, "Запуск мода с картой", true, aTabMap.Tab )
	aTabMap.CurMap = guiCreateLabel ( 0.46, 0.15, 0.40, 0.035, "Текущая карта: N/A", true, aTabMap.Tab )
	aTabMap.CurGamemode = guiCreateLabel ( 0.46, 0.2, 0.40, 0.035, "Текущий мод: N/A", true, aTabMap.Tab )
	aTabMap.NextMap = guiCreateButton ( 0.45, 0.35, 0.3, 0.04, "Поставить след карту", true, aTabMap.Tab )
	aTabMap.Delete = guiCreateButton ( 0.45, 0.40, 0.3, 0.04, "Удалить карту", true, aTabMap.Tab )
	aTabMap.Revert = guiCreateButton ( 0.45, 0.45, 0.3, 0.04, "Вернуть карту", true, aTabMap.Tab )
	guiSetVisible(aTabMap.Delete, false)
	guiSetVisible(aTabMap.Revert, false)
	aTabMap.RefreshList = guiCreateButton ( 0.03, 0.91, 0.35, 0.04, "Обновить список", true, aTabMap.Tab )
...
function loadMaps(gamemodeMapTable, gamemode, map)
	guiSetText(aTabMap.CurMap,"Текущая карта: "..map)
	guiSetText(aTabMap.CurGamemode,"Текущий мод: "..gamemode)
	...
	if button == "left" then
		if ( getElementParent ( source ) == aTabMap.Tab ) then
			...
			if not guiGridListGetSelectedItem ( aTabMap.MapList ) == -1 then
				aMessageBox ( "Ошибка", "Карта не выбрана!" )
			end
			...
			elseif source == aTabMap.Delete then
				aMessageBox ( "Вопрос", "Удалить '"..mapName.."'?", "triggerServerEvent(\"deleteRevertMap_s\", getLocalPlayer(), true, \""..mapResName.."\", \""..mapName.."\")" )
...
addEventHandler("deleteRevertMap_c", getLocalPlayer(),
	function(success, delete, mapName)
		if success then
			...
			triggerServerEvent("getMaps_s", getLocalPlayer(), true)
			if delete then
				aMessageBox ( "Информация", "Карта '"..mapName.."' успешно удалена!" )
			else
				aMessageBox ( "Информация", "Карта '"..mapName.."' успешно восстановлена!" )
			end
		else
			aMessageBox ( "Ошибка", "невозможно удалить/восстановить!" )
		end
	end
)
