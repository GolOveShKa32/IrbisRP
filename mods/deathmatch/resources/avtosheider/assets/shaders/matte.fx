  ...
  float sAdd = 0;  
  float sMul = 0; 
  float sCutoff : CUTOFF = 2;         // 0 - 1
  float sPower : POWER  = 2;            // 1 - 5
  float sNorFac = 0;
  float sProjectedXsize = 0;
  float sProjectedXvecMul = 0;
  float sProjectedXoffset = 0;
  float sProjectedYsize = 0;
  float sProjectedYvecMul = 0;
  float sProjectedYoffset = 0;
...
float4 PixelShaderFunction(PSInput PS) : COLOR0
{
    ...
    //
    float3 vNp1 = microflakePerturbationA * normalPerturbation * vNormal ;
    ...
    float3 vNp2 = microflakePerturbation * vNormal;
   ...
   float3 vView = PS.View;
	...
	if(isChrome == 1) {
		for (float i = 1; i < 3; i++) {
			envMap += tex2D(ReflectionSampler, vReflection.xy + (i * 6.0020));
			envMap += tex2D(ReflectionSampler, vReflection.xy - (i * 6.0020));
		}
